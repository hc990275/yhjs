...
(function() {
    ...
    header.innerHTML = "⚖️ 原始数据 V7.9✕ 关闭";
    ...
    controlBar.innerHTML = '▶️ 记录⏹️ 停止📥 导出未记录';
    ...
    statsBar.innerHTML = '📊 0条⏱️ 00:00:00● 录制中';
    ...
    analysisHeader.innerHTML = "🎯 分析框 V7.9 增强版拖动移动 | 边角缩放";
    ...
    function updateAnalysisPanel(chartData) {
        ...
        screens.forEach(function(screen) {
            ...
            if (config.simpleMode) {
                ...
                html += "";
                html += "中轨: " + railLength + "";
                html += "" + momStatus.side + ": " + momentum.toFixed(3) + "";
                html += "快线: " + fastLine.toFixed(3) + "";
                html += "" + momStatus.side + momStatus.status + "";
                html += "";
                
                if (result.energyWarning) {
                    html += "" + result.energyWarning + "";
                }
                ...
                html += "" + screen.name + "";
                if (config.analysisMode === 'period') {
                    html += "周期:" + (config.periodTime/60000).toFixed(1) + "分";
                }
                html += "" + fastStatus.text + "";
                html += "";
                ...
                if (macdChart && macdChart.data && macdChart.data.length >= 11) {
                    ...
                    html += "";
                    html += "" + crossText + "";
                    html += "快线" + fastStatus.text + "";
                    html += "" + momStatus.side + momStatus.status + "";
                    html += "";
                    
                    html += "";
                    html += "动能(9)" + momentum.toFixed(3) + "(" + (momentumChange >= 0 ? '+' : '') + momentumChange.toFixed(4) + ")";
                    html += "快线(10)" + fastLine.toFixed(3) + "("+  (fastLineChange >= 0 ? '+' : '') + fastLineChange.toFixed(4) + ")";
                    html += "慢线(11)" + slowLine.toFixed(3) + "";
                    html += "";
    ...
    function updatePanel() {
        ...
        if (widgets.length < 2) {
            content.innerHTML = "⚠️ 需要至少2个分屏";
            analysisContent.innerHTML = "⚠️ 等待分屏...";
            return;
        }
